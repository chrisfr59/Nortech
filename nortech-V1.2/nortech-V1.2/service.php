...
<div>
	<table>
		<thead>
			<tr>
			<th>N° service</th><th>Service</th><th>Ville</th><th>Dépenses </th><th>Mise à jour</th>
			</tr>
		</thead>
		<tbody>
	<?php
	...
	echo '<tr>';
	echo '<td>'.$resultat['noServ'].'</td><td><div id="msg" onmouseover="afficher(infos[2]);" onmouseout="masquer();">'.$resultat['service'].'</div></td><td>'.$resultat['ville'].'</td><td>'.$resultat['SUM(ifnull(f.montant,0))'].' €'.'</td><td><a class="button" href="update.php?section=service&numServ='.$resultat['noServ'].'">Update</a></td>';
	echo '</tr>';
        }
